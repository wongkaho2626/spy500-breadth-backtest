...
# (look-ahead) fill. The annual rebalance is a scheduled event (composition known
# in advance), so it fills same-day at the chosen price without a lag.
...
def run_strategy(
    ...
    bars later, buying/selling the stock basket at the fill bar's open
    (fill_on="open") or close. lag=0 requires fill_on="close" (legacy same-day
    look-ahead). Mark-to-market always uses stock closes."""
    if fill_on == "open" and execution_lag < 1:
        raise ValueError("fill_on='open' requires execution_lag >= 1 (open precedes close)")
    # Price source used for FILLS (buys/sells/rebalance); MTM always uses closes.
