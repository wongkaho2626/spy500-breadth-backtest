...
# PPI gates: block buys when inflation is extreme (Fed constrained, no rate-cut backstop);
# only confirm divergence exits when PPI shows meaningful inflationary pressure at the top.
...
def load_data() -> pd.DataFrame:
    ...
    # Load monthly PPI YoY and forward-fill to daily trading calendar
    ...
    # Only exit on divergence when PPI shows inflationary pressure (complacent topping)
...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    # Shade zones where buys are blocked (PPI too high)
    ...
    out_path = DATA_DIR / "qqq_ppi_performance.png"
    plt.tight_layout()
