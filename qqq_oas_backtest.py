...
OAS_BUY_MIN  = 4.5   # % — require credit stress before buying
OAS_SELL_CAP = 5.0   # % — bearish_div only fires when credit is calm (low OAS = complacent top)
...
def load_data() -> pd.DataFrame:
    ...
    # Load monthly OAS and forward-fill to daily trading calendar
    ...
    # Bearish divergence base: price up + breadth falling + breadth not too high
    ...
    # Only exit on divergence when credit is calm (low OAS = complacent topping pattern)
    merged["bearish_div"] = div_base & (merged["oas"] < OAS_SELL_CAP)
...
def run_strategy(df: pd.DataFrame) -> tuple[pd.Series, list[dict], dict | None]:
    ...
    for date, row in df.iterrows():
        ...
        if position == "OUT" and breadth < BUY_THRESHOLD and oas >= OAS_BUY_MIN:
            ...
            sell_reason = None
            if bearish_div:
                sell_reason = "bearish-div+low-oas"
            if sell_reason:
...
def print_trades(trades: list[dict], open_trade: dict | None = None) -> None:
    ...
    header = (f"\n{'#':>3}  {'Entry':10}  {'Exit':10}  {'Held':>7}  {'Entry $':>8}  {'Exit $':>8}"
              f"  {'OAS@Buy':>7}  {'OAS@Sell':>8}  {'Return':>8}  {'Drawdown':>9}  "
    ...
    for i, t in enumerate(trades, 1):
        ...
        print(
            ...
            f"{t['entry_oas']:>6.2f}%  {t['exit_oas']:>7.2f}%  "
    ...
    if open_trade:
        ...
        print(
            ...
            f"{open_trade['entry_oas']:>6.2f}%  {open_trade['current_oas']:>7.2f}%  "
...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    fig.suptitle(
        f"QQQ: Breadth + OAS Strategy\n"
        f"Buy: breadth<{BUY_THRESHOLD} AND oas≥{OAS_BUY_MIN}%  |  "
        f"Sell: bearish-div (oas<{OAS_SELL_CAP}%)\n"
    ...
    ax1.plot(strategy.index,  strategy,  label="Breadth+OAS Strategy", color="#FF5722", linewidth=1.5)
    ...
    ax2.axhline(BUY_THRESHOLD, color="green", linestyle="--", linewidth=1.2,
                label=f"Buy: breadth<{BUY_THRESHOLD}")
    ...
    # Panel 3: OAS
    ax3.plot(df.index, df["oas"], color="#E65100", linewidth=1.2, label="OAS Single-B HY (%)")
    ax3.axhline(OAS_BUY_MIN,  color="green", linestyle="--", linewidth=1.2,
                label=f"Buy min OAS: {OAS_BUY_MIN}%")
    ax3.axhline(OAS_SELL_CAP, color="red",   linestyle=":",  linewidth=1.2,
                label=f"Div sell cap: {OAS_SELL_CAP}%")
    ax3.fill_between(df.index, 0, df["oas"],
                     where=df["oas"] >= OAS_BUY_MIN, color="green", alpha=0.08,
                     label=f"Buy-eligible OAS zone (≥{OAS_BUY_MIN}%)")
    if entry_dates:
        ax3.scatter(entry_dates, df["oas"].reindex(entry_dates, method="nearest"),
                    marker="^", color="green", s=60, zorder=5)
    if exit_dates:
        ax3.scatter(exit_dates, df["oas"].reindex(exit_dates, method="nearest"),
                    marker="v", color="red", s=60, zorder=5)
    ax3.set_ylabel("OAS Spread (%)")
    ax3.set_xlabel("Date")
    ...
    out_path = DATA_DIR / "qqq_oas_performance.png"
    plt.tight_layout()
...
def main() -> None:
    ...
    print(
        f"Strategy   : buy breadth<{BUY_THRESHOLD} AND oas≥{OAS_BUY_MIN}% | "
        f"sell bearish-div (window={DIVERGENCE_WINDOW}d, QQQ+{DIVERGENCE_PRICE_RISE}%, "
        f"breadth↓≥{DIVERGENCE_BREADTH_FALL}pts, cap<{DIVERGENCE_BREADTH_CAP}, oas<{OAS_SELL_CAP}%)"
    )
