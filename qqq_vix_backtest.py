"""
QQQ Breadth + VIX Strategy
===========================
Hypothesis: require VIX < VIX_SELL_CAP on the bearish-divergence exit so the strategy
only exits when the market is *calm* (complacency = topping signal confirmed by low vol).
High-VIX divergences (e.g. European debt crisis 2011, COVID Feb 2020) are panic-driven
corrections and may resolve without a regime change, so the filter lets the strategy ride
through them.

Grid-search result summary vs baseline (qqq_backtest.py):
  Baseline (no VIX)        CAGR 19.2%  Sharpe 0.99  MDD -49.4%  8 trades
  Sell VIX<30 (this file)  CAGR 18.2%  Sharpe 0.94  MDD -49.4%  6 trades
  Sell VIX<25              CAGR 16.0%  Sharpe 0.83  MDD -49.4%  4 trades
  Sell VIX<20              CAGR 15.9%  Sharpe 0.81  MDD -49.4%  3 trades
  Buy VIX≥25               CAGR 16.7%  Sharpe 0.90  MDD -49.4%  7 trades
  Buy VIX≥20               CAGR 19.2%  Sharpe 0.99  MDD -49.4%  8 trades  (identical)

Conclusion: VIX thresholding does NOT improve the strategy. The breadth signal already
captures the panic/fear condition effectively. VIX buy filters block good trades
(2018-12-17 VIX=24.5, 2023-10-27 VIX=21.3). VIX sell filters prevent timely exits
and degrade CAGR. The 2011 exit (VIX=31) blocked by cap<30 cost performance because
the re-entry in qqq_backtest captured the subsequent dip lower. This file is kept as
an experiment log; the VIX panel in the chart provides useful visual context.
"""
import numpy as np
import pandas as pd
...
# Best VIX configuration found: sell-side cap only, buy side unrestricted.
# Even this underperforms the baseline — see module docstring for full grid results.
VIX_BUY_MIN  = None   # set to a float (e.g. 25.0) to require elevated VIX at entry
VIX_SELL_CAP = 30.0   # only fire bearish-div exit when VIX < this (calm market)
...
def load_data() -> pd.DataFrame:
    ...
    for df in (qqq_raw, breadth_raw, b50_raw, vix_raw):
        ...
        if position == "OUT" and breadth < BUY_THRESHOLD and b50 < BUY_50_THRESHOLD and vix_buy_ok:
            ...
            if bearish_div:
                ...
                trades.append({
                    ...
                    "sell_reason":      "bearish-div+low-vix",
                })
                position = "OUT"
...
def print_vix_analysis(df: pd.DataFrame) -> None:
    ...
    triggers = [
        ("Buy",  "2008-01-17"), ("Buy",  "2011-09-22"), ("Buy",  "2015-08-24"),
        ("Buy",  "2018-12-17"), ("Buy",  "2020-02-28"), ("Buy",  "2020-03-09"),
        ("Buy",  "2022-05-19"), ("Buy",  "2023-10-27"), ("Buy",  "2025-04-04"),
        ("Sell", "2011-09-16"), ("Sell", "2014-08-07"), ("Sell", "2018-03-22"),
        ("Sell", "2020-02-25"), ("Sell", "2020-03-02"), ("Sell", "2021-11-30"),
        ("Sell", "2023-05-01"), ("Sell", "2024-12-18"),
    ]
...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    out_path = DATA_DIR / "qqq_vix_performance.png"
    plt.tight_layout()
...
def main() -> None:
    ...
    print(
        ...
        f"QQQ+{DIVERGENCE_PRICE_RISE}%, breadth200↓≥{DIVERGENCE_BREADTH_FALL}pts, cap<{DIVERGENCE_BREADTH_CAP})"
    )
    ...
    print("\n*** NOTE: VIX does not improve the baseline breadth strategy. ***")
    print("    Baseline (qqq_backtest.py): CAGR 19.2%, Sharpe 0.99, 8 trades")
    print(f"    This file (Sell VIX<{VIX_SELL_CAP}):    CAGR 18.2%, Sharpe 0.94, 6 trades")
