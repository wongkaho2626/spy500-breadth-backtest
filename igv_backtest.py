...
def load_data() -> pd.DataFrame:
    ...
    price_past   = merged["igv_price"].shift(DIVERGENCE_WINDOW)
    breadth_past = merged["breadth"].shift(DIVERGENCE_WINDOW)
    merged["bearish_div"] = (
        ((merged["igv_price"] - price_past) / price_past * 100 >= DIVERGENCE_PRICE_RISE) &
...
def run_strategy(df: pd.DataFrame) -> tuple[pd.Series, list[dict], dict | None]:
    ...
    open_trade = None
    if position == "IN":
        last_price = df["igv_price"].iloc[-1]
...
def run_benchmark(df: pd.DataFrame) -> pd.Series:
    first_price = df["igv_price"].iloc[0]
    return (INITIAL_CAPITAL * df["igv_price"] / first_price).rename("benchmark")
...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    fig.suptitle(
        f"IGV: Breadth Strategy\n"
        f"Buy: breadth200<{BUY_THRESHOLD} AND breadth50<{BUY_50_THRESHOLD}  |  "
        f"Sell: bearish-div (IGV+{DIVERGENCE_PRICE_RISE}% & breadth200↓{DIVERGENCE_BREADTH_FALL}pts"
        f" & breadth200<{DIVERGENCE_BREADTH_CAP})\n"
        f"Starting capital: ${INITIAL_CAPITAL:,.0f}",
        fontsize=11, fontweight="bold"
    )

    ax1.plot(benchmark.index, benchmark, label="Buy & Hold IGV", color="#2196F3", linewidth=1.5)
    ...
    out_path = DATA_DIR / "igv_performance.png"
    plt.tight_layout()
...
def main() -> None:
    ...
    print(
        ...
        f"(window={DIVERGENCE_WINDOW}d, IGV+{DIVERGENCE_PRICE_RISE}%, "
