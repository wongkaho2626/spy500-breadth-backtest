"""
Combined Portfolio: TQQQ 5% / NDX Top-1 25% / QQQ 70%

Initial capital : $153,402
Annual funding  : $26,880 on April 6 each year (starting 2012)
  - If in a trade : deploy T5/N25/Q70 into current positions immediately
  - If out        : hold as pending cash, deploy on next buy signal
Rebalancing     : T5/N25/Q70 at every trade entry
Signals (qqq_backtest rules):
  BUY : S&P 500 breadth200 < 26%
  SELL: NDX price rose >= 3% over 60d AND breadth fell >= 20pts AND breadth < 60%

All three legs use the unified NDX-based divergence signal so they
enter and exit simultaneously.

Exports (in same directory):
  combined_t5n25q70_metrics.csv
  combined_t5n25q70_combined.csv   — combined portfolio trade log
  combined_t5n25q70_tqqq.csv       — TQQQ leg trade log
  combined_t5n25q70_ndxa.csv       — NDX Top-1 leg trade log
  combined_t5n25q70_qqqb.csv       — QQQ-B (NDX proxy) leg trade log
"""
import warnings
...
def load_tqqq(breadth: pd.DataFrame) -> pd.DataFrame:
    ...
    tqqq.index = pd.to_datetime(tqqq.index).tz_localize(None)
    opn = raw["Open"]
    if isinstance(opn, pd.DataFrame):
        opn = opn.iloc[:, 0]
    opn = opn.rename("open")
    opn.index = pd.to_datetime(opn.index).tz_localize(None)
    merged     = tqqq.to_frame().join(opn, how="left").join(breadth, how="left")
...
def run_portfolio(
    ...
    """
    Simulate the 3-leg rebalanced portfolio with annual contributions.
    Uses unified NDX-based sell signal so all legs exit simultaneously.
    Returns (equity_series, comb_trades, tqqq_trades, ndxa_trades, qqqb_trades).
    """
    common_idx    = df_tqqq.index.intersection(df_ndx.index).sort_values()
    ...
    # ── TQQQ leg state (shares-based) ────────────────────────────────────────
    t_pos         = "OUT"
    t_shares      = 0.0
    t_cash        = INITIAL_CAPITAL * TQQQ_PCT
    t_entry_date: pd.Timestamp | None = None
    t_entry_val   = 0.0
    t_entry_price = 0.0
    t_mid_contrib = 0.0
    t_price_low   = 0.0
    ...
    tqqq_trades:  list[dict] = []
    ndxa_trades:  list[dict] = []
    qqqb_trades:  list[dict] = []

    def t_mktval(price: float) -> float:
        return t_shares * price if t_pos == "IN" else t_cash
    ...
    for i, date in enumerate(common_idx):
        if date not in df_tqqq.index or date not in df_ndx.index:
            continue

        row_t   = df_tqqq.loc[date]
        row_n   = df_ndx.loc[date]
        t_price = float(row_t["price"])      # closes: mark-to-market
        n_price = float(row_n["price"])
        # Fill prices: this bar's open when available (next-day-open model).
        t_open  = float(row_t["open"]) if ("open" in row_t and not pd.isna(row_t["open"])) else t_price
        n_open  = float(row_n["open"]) if ("open" in row_n and not pd.isna(row_n["open"])) else n_price
        t_fill  = t_open if fill_on == "open" else t_price
        n_fill  = n_open if fill_on == "open" else n_price
        # Signal bar: the NDX row `execution_lag` bars ago (known at that close).
        ...
        # Unified NDX-based signal for all legs
        ...
        if date in contrib_dates:
            if not trade_open:
                ...
                t_add = ANNUAL_CONTRIB * TQQQ_PCT
                n_add = ANNUAL_CONTRIB * NDXA_PCT
                q_add = ANNUAL_CONTRIB * QQQB_PCT

                if t_pos == "IN":
                    new_shares = (t_add - COMMISSION) / (t_price * (1 + SLIPPAGE))
                    t_shares  += new_shares
                    t_mid_contrib += t_add
                else:
                    t_cash += t_add
        ...
        if n_pos == "IN" and prev_year is not None and year != prev_year:
            ...
            cur = _basket_value(n_basket, stock_fill, date)
            if cur > 0 and new_comp:
                after_sell = cur * (1 - SLIPPAGE) - COMMISSION
                n_basket   = _build_basket(after_sell, new_comp, stock_fill, date)
        prev_year = year
        ...
        if bearish_div and trade_open:
            if t_pos == "IN":
                exit_t     = t_shares * t_fill * (1 - SLIPPAGE) - COMMISSION
                deployed_t = t_entry_val + t_mid_contrib
                ret_t      = (exit_t - deployed_t) / deployed_t * 100 if deployed_t > 0 else 0.0
                tqqq_trades.append({
                    "entry_date":   t_entry_date,
                    "exit_date":    date,
                    "entry_value":  t_entry_val,
                    "mid_contribs": t_mid_contrib,
                    "exit_value":   exit_t,
                    "net_gain":     exit_t - deployed_t,
                    "return_pct":   ret_t,
                    "price_dd":     (t_price_low - t_entry_price) / t_entry_price * 100 if t_entry_price > 0 else 0.0,
                    "port_peak":    t_port_peak,
                    "port_trough":  t_trough,
                    "port_dd":      t_worst_dd,
        ...
        if t_pos == "IN":
            t_price_low = min(t_price_low, t_price)
            t_port_peak = max(t_port_peak, t_val)
            dd = (t_val - t_port_peak) / t_port_peak * 100
            if dd < t_worst_dd:
                t_worst_dd = dd;  t_trough = t_val
        ...
        if not trade_open and (washout_buy or trend_buy):
            ...
            t_alloc = total_capital * TQQQ_PCT
            n_alloc = total_capital * NDXA_PCT
            q_alloc = total_capital * QQQB_PCT

            # Enter TQQQ
            t_buy         = t_alloc - COMMISSION
            t_shares      = t_buy / (t_fill * (1 + SLIPPAGE))
            t_entry_val   = t_buy
            t_entry_price = t_fill
            t_mid_contrib = 0.0
            t_price_low   = t_fill
            t_port_peak   = t_buy;  t_worst_dd = 0.0;  t_trough = t_buy
            t_entry_date  = date;   t_pos = "IN";      t_cash = 0.0
    ...
    if trade_open:
        last_t = float(df_tqqq["price"].iloc[-1])
        last_n = float(df_ndx["price"].iloc[-1])

        if t_pos == "IN":
            exit_t_est = t_shares * last_t * (1 - SLIPPAGE)
            deployed_t = t_entry_val + t_mid_contrib
            tqqq_trades.append({
                "entry_date":   t_entry_date,
                "exit_date":    None,
                "entry_value":  t_entry_val,
                "mid_contribs": t_mid_contrib,
                "exit_value":   exit_t_est,
                "net_gain":     exit_t_est - deployed_t,
                "return_pct":   (exit_t_est - deployed_t) / deployed_t * 100 if deployed_t > 0 else 0.0,
                "price_dd":     (t_price_low - t_entry_price) / t_entry_price * 100 if t_entry_price > 0 else 0.0,
                "port_peak":    t_port_peak,
                "port_trough":  t_trough,
                "port_dd":      t_worst_dd,
    ...
    return (pd.Series(combined_values, name="portfolio"),
            comb_trades, tqqq_trades, ndxa_trades, qqqb_trades)
...
def export_csvs(
    ...
    prefix = DATA_DIR / "combined_t5n25q70"
...
def main() -> None:
    ...
    df_tqqq = load_tqqq(breadth)
    df_ndx  = load_ndx(breadth)

    common_start = max(df_tqqq.index[0], df_ndx.index[0])
    df_tqqq = df_tqqq[df_tqqq.index >= common_start]
    df_ndx  = df_ndx[df_ndx.index >= common_start]

    print(f"Backtest period : {common_start.date()} → {df_tqqq.index[-1].date()}")
    print(f"Initial capital : ${INITIAL_CAPITAL:,.0f}")
    print(f"Annual funding  : ${ANNUAL_CONTRIB:,.0f} on April {CONTRIB_DAY} each year "
          f"(starting {CONTRIB_START_YEAR})")
    print(f"Allocation      : TQQQ {TQQQ_PCT:.0%} / NDX-Top1 {NDXA_PCT:.0%} / QQQ {QQQB_PCT:.0%}")
    print()
    ...
    equity, comb_trades, tqqq_trades, ndxa_trades, qqqb_trades = run_portfolio(
        df_tqqq, df_ndx, h1, stock_prices, stock_opens=stock_opens
    )

    last_date = df_tqqq.index[-1]

    # Per-leg equity reconstruction for standalone metrics
    ...
    m_tqqq = compute_metrics("TQQQ (5%)",      _leg_equity(tqqq_trades, INITIAL_CAPITAL * TQQQ_PCT), tqqq_trades)
    ...
    print_metrics_table([m_comb, m_tqqq, m_ndxa, m_qqqb])

    print_combined_log(comb_trades)
    print_leg_log("TQQQ Leg",      tqqq_trades, last_date, show_holdings=False)
    ...
    print()
    export_csvs([m_comb, m_tqqq, m_ndxa, m_qqqb],
                comb_trades, tqqq_trades, ndxa_trades, qqqb_trades)
