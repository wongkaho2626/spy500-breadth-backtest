...
Same signal set as qqq_backtest.py, applied to SOXX:
...
def _load_breadth() -> pd.DataFrame:
    ...
    # S5TH uses Investing.com format: Date (MM/DD/YYYY), Price
...
def print_sell_proximity(df: pd.DataFrame, open_trade: dict | None) -> None:
    ...
    div_met     = price_met and breadth_met and cap_ok
    ...
    print(f"  {sep}")
    verdict = "YES — sell signal ACTIVE" if div_met else "NO  — not yet triggered"
...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    fig.suptitle(
        "SOXX Semiconductor ETF Breadth Strategy\n"
    ...
    ax2.plot(df.index, df["breadth"], color="#7B1FA2", linewidth=1.0,
             label="% S&P 500 Stocks Above 200-Day MA (S5TH)")
    ...
    ax3.plot(df.index, df["price"], color="#546E7A", linewidth=1.0, label="SOXX")
    if all_entries:
    ...
    out = DATA_DIR / "soxx_performance.png"
    plt.tight_layout()
