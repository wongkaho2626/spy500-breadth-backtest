...
def plot_results(df, strategy, benchmark, trades, open_trade) -> None:
    ...
    fig.suptitle(
        f"SOXX: Breadth Strategy\n"
        f"Buy: breadth200<{BUY_THRESHOLD} AND breadth50<{BUY_50_THRESHOLD}  |  "
        f"Sell: bearish-div (SOXX+{DIVERGENCE_PRICE_RISE}% & breadth200↓{DIVERGENCE_BREADTH_FALL}pts"
    ...
    ax2.axhline(DIVERGENCE_BREADTH_CAP, color="darkorange", linestyle=":",
                linewidth=1.2, label=f"Div cap: {DIVERGENCE_BREADTH_CAP}")
    ...
    out_path = DATA_DIR / "soxx_performance.png"
    plt.tight_layout()
...
def main() -> None:
    ...
    print(
        ...
        f"(window={DIVERGENCE_WINDOW}d, SOXX+{DIVERGENCE_PRICE_RISE}%, "
